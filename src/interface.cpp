...
Interface::Interface(LCD* _lcd) :
		lcd_(_lcd), up_(defaults::pines.btn_up, true), down_(
				defaults::pines.btn_down, true), right_(
				defaults::pines.btn_right, true), left_(
				defaults::pines.btn_left, true), ok_(defaults::pines.btn_ok,
				true), esc_(defaults::pines.btn_esc, true) {
...
void Home::down() {
	interface_->move_to_screen(External::instance());
}
void Home::up() {
	interface_->move_to_screen(System::instance());
}
void Home::ok() {
...
void Actuators::ok() {
	interface_->move_to_screen(ActuatorsEdit::instance());
}
...
void WhiteLed::ok() {
	interface_->move_to_screen(WhiteLedEdit::instance());
}
void WhiteLed::update_screen() {
	to_screen(config::white, "WHITE");
}

WhiteLedEdit::WhiteLedEdit() :
		ScreenPhotoperiodEdit(config::white.periods) {
}
WhiteLedEdit* WhiteLedEdit::instance() {
	static WhiteLedEdit screen;
	return &screen;
}
void WhiteLedEdit::ok() {
	interface_->move_to_screen(WhiteLed::instance());
}
...
void WhiteLedEdit::update_screen() {
	to_screen(config::white, "WHITE");
}
...
void RedLed::ok() {
	interface_->move_to_screen(RedLedEdit::instance());
}
void RedLed::update_screen() {
	to_screen(config::red, "RED");
}

RedLedEdit::RedLedEdit() :
		ScreenPhotoperiodEdit(config::red.periods) {
}
...
void GreenLed::ok() {
	interface_->move_to_screen(GreenLedEdit::instance());
}
void GreenLed::update_screen() {
	to_screen(config::green, "GREEN");
}

GreenLedEdit::GreenLedEdit() :
		ScreenPhotoperiodEdit(config::green.periods) {
}
GreenLedEdit* GreenLedEdit::instance() {
	static GreenLedEdit screen;
	return &screen;
}
void GreenLedEdit::ok() {
	interface_->move_to_screen(GreenLed::instance());
}
...
void GreenLedEdit::update_screen() {
	to_screen(config::green, "GREEN");
}
...
void BlueLed::ok() {
	interface_->move_to_screen(BlueLedEdit::instance());
}
void BlueLed::update_screen() {
	to_screen(config::blue, "BLUE");
}

BlueLedEdit::BlueLedEdit() :
		ScreenPhotoperiodEdit(config::blue.periods) {
}
...
void BlueLedEdit::update_screen() {
	to_screen(config::blue, "BLUE");
}
...
void UltraVioletLedEdit::update_screen() {
	to_screen(config::uv, "UV");
}
...
void Feeding::ok() {
	interface_->move_to_screen(FeedingEdit::instance());
}
...
FeedingEdit::FeedingEdit() :
		editing_ { { &config::feeding.breakfast.hour, 16, 0, 23 }, {
...
void PH::down() {
	interface_->move_to_screen(TDS::instance());
}
void PH::ok() {
	interface_->move_to_screen(PHEdit::instance());
}
...
ScreenPhotoperiodEdit::ScreenPhotoperiodEdit(actuators::PhotoPeriod p[]) :
		editing_ {
				// Rise
				{ &p[0].init.hour, 8, 1, 23 }, { &p[0].init.minute, 11, 1, 59 },
				{ &p[0].end.hour, 16, 1, 23 }, { &p[0].end.minute, 19, 1, 59 },
				// On
				{ &p[1].init.hour, 8, 2, 23 }, { &p[1].init.minute, 11, 2, 59 },
				{ &p[1].end.hour, 16, 2, 23 }, { &p[1].end.minute, 19, 2, 59 },
				// Fall
				{ &p[2].init.hour, 8, 3, 23 }, { &p[2].init.minute, 11, 3, 59 },
				{ &p[2].end.hour, 16, 3, 23 }, { &p[2].end.minute, 19, 3, 59 }, },
		current_ {0} {
}
