...
void LedStripImpl::setup() { pinMode(led_->pin, OUTPUT); }
    ...
    for (auto period : led_->periods) {
        if (period.in_period(ahora)) {
            ...
            case (PeriodType::fall):
                status_.pwm =
                    255 - (uint8_t)map(ahora.minutes(), period.init.minutes(),
                                       period.end.minutes(), 0, 255);
                break;

            case (PeriodType::rise):
                status_.pwm =
      ...
      break; // end search fir current period
        }
    }
    analogWrite(led_->pin, status_.pwm);
}
